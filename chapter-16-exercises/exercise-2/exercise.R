...
ggplot(data = diamonds) +
  geom_col(
    mapping = aes(x = price, y = cut, fill = clarity)
  )

# Draw the same chart again, but with each element positioned to "fill" the y axis
ggplot(data = diamonds) +
  geom_col(
    mapping = aes(x = carat, y = price, fill = clarity), position = "fill"
  )

# Draw the same chart again, but with each element positioned to "dodge" each other
ggplot(data = diamonds) +
  geom_col(
    mapping = aes(x = carat, y = price, fill = clarity), position = "dodge"
  )
...
ggplot(data = diamonds) +
  ...
  scale_y_log10()
# The prices are distributed better
...
ggplot(data = diamonds) +
  ...
  scale_x_log10() +
  scale_y_log10()
# Data become continuous like a line
